...
class Dbbak {
	...
	{
		$sqlname=time();//文件夹名、文件前缀
		$this->createdir($this->dataDir.$sqlname);//创建文件夹
		$dir=$this->dataDir.$sqlname.'/';//存放路径
		...
		if($subsection == 0)
		{
			...
			$fileName = $dir.$sqlname.'_all.sql.php';
		...
		else
		{
			...
			{
				$fileName = $dir.$sqlname.'_part'.$i.'.sql.php';
