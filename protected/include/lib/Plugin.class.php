...
class Plugin
{
	...
	static public $_pluginsets=array();//存放插件参数
	static public $_plugins=array();//存放所有插件的方法
	...
	static public function init($suffix='',$config=array())
	{
        ...
        self::$config['PLUGIN_INFOFILE']=isset($config['PLUGIN_INFOFILE'])?$config['PLUGIN_INFOFILE']:'setting.php';//插件参数文件
		//插件目录不以“/”结尾，加上“/”
   ...
   //插件参数读取
    static public function setting($plugin_name,$name = '')
    {
        $setting = array();
        $setting = require_once(self::$config['PLUGIN_PATH'].$plugin_name.'/'.self::$config['PLUGIN_INFOFILE']);
        if(!is_array($setting)) return false;
        if (empty($name))
            return $setting; //返回所有配置
        else  
            return isset($setting[$name]) ? $setting[$name] : false; //返回指定配置值
    }
    
	//遍历获取所有插件
	static public function get()
	{
		if(!empty(self::$_plugins)&&!empty(self::$_pluginsets))
			return true;
        ...
        while (false !== ($file = readdir($handle))) 
        {
			  ...
			  if (is_dir( self::$config['PLUGIN_PATH'].$file )&&$file!='.'&&$file!='..' )
			  {
					...
					if(self::_check($plugin_name)==false)
						continue;
					if($pluginset=self::setting($plugin_name))
					      self::$_pluginsets[$plugin_name]=$pluginset;//获取插件配置
	...
	*/
	static public function hook($action_name,$plugin_name='',$data=NULL)
	{
		...
		if(is_array(self::$_plugins))
		{
			...
			{
			  return self::run($plugin_name,$action_name,$data);//运行符合条件的插件
			}
			else
			{
