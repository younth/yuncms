...
class Auth
{
	...
	static public function init($config=array())
	{
		...
		//登录地址
		self::$config['AUTH_LOGIN_URL']=isset($config['AUTH_LOGIN_URL'])?$config['AUTH_LOGIN_URL']:url('index/login');
		//不需要认证的模块，对后台认证有效
		self::$config['AUTH_LOGIN_NO']=isset($config['AUTH_LOGIN_NO'])?$config['AUTH_LOGIN_NO']:array('index'=> array('login','logout','verify'),'common'=>'*');
		//session前缀
		...
		self::$config['AUTH_TABLE']=isset($config['AUTH_TABLE'])?$config['AUTH_TABLE']:array(
									'group'=>array(
											'name'=>'group',
											'field'=>array('id'=>'id','power'=>'power'),
		),
									'resource'=>array(
											'name'=>'method',
											'field'=>array('id'=>'id','rootid'=>'rootid','pid'=>'pid','operate'=>'operate'),
		),
		);
	}
	...
	//权限认证检查
	static public function check($config=array())
	{
		self::init($config);
		$module=CONTROLLER_NAME;//获取当前执行的模块
		$action=ACTION_NAME;//获取当前执行的操作
		//print_r($_SESSION);
		...
		if(false==self::checkLogin())
		{
		    ...
		    if(($power==-1)||($power[$module][$action]==-1)) return true;//认证通过
		    else self::noPower();
	...
	//自动获取模块信息
	static public function getModule($config=array(),$module_path='',$module_suffix='')
	{
		self::init($config);
		//完成数据库表名和字段名映射
		...
		$resource_field_rootid=self::$config['AUTH_TABLE']['resource']['field']['rootid'];//资源表 rootid字段
		...
		//如果没有用户组和资源数据表，则自动创建
		$sql_group="CREATE TABLE IF NOT EXISTS `".config('DB_PREFIX').$table_group."` (
  ...
  PRIMARY KEY (`id`)
		...
		$sql_resource="CREATE TABLE IF NOT EXISTS `".config('DB_PREFIX').$table_resource."` (
  `{$resource_field_id}` int(10) unsigned NOT NULL auto_increment,
  `{$resource_field_rootid}` int(10) unsigned NOT NULL,
  `{$resource_field_pid}` int(10) unsigned NOT NULL,
  `{$resource_field_operate}` varchar(255) NOT NULL,
  `name` varchar(255) NOT NULL,
  `ifmenu` tinyint(1) NOT NULL COMMENT '是否菜单显示',
  PRIMARY KEY  (`id`),
  KEY `pid` (`pid`)
) ENGINE=MyISAM  DEFAULT CHARSET=utf8 AUTO_INCREMENT=1;";
		model($table_group)->query($sql_group);
		model($table_resource)->query($sql_resource);
			
		$module_path=empty($module_path)?BASE_PATH.'apps/admin/controller/':$module_path;
		$module_suffix=empty($module_suffix)?'Controller.php':$module_suffix;
		$return_data=array();
		...
		if(!empty($return_data))
		{
			...
			foreach($return_data as $key=>$value)
			{
				...
				$info=model($table_resource)->find($condition);
				if(empty($info))
				{
					$pid=model($table_resource)->insert($data);
					$newdata[$resource_field_rootid]=$pid;
					model($table_resource)->update($condition,$newdata);
				}
				else
				{
				...
				if(is_array($value))
				{
					...
					{				
						...
						$data[$resource_field_operate]=$condition[$resource_field_operate]=$key;
						$data[$resource_field_rootid]=$pid;

						$info=model($table_resource)->find($condition);
						if(empty($info))
						{
							model($table_resource)->insert($data);
						}
					}
				}
			}
	...
	static public function getGroupPower($group_id='')
	{
		...
		$group=model($table_group)->find($condition);
		...
		else
		{
			...
			$resource=model($table_resource)->select();
			//资源为空，则返回false
			if(empty($resource))
			return false;
			...
			$power=$apppower=$resource2=array();
			...
			{
				//筛选该用户组拥有的权限信息
				if(in_array($vo[$resource_field_id],$power_value)){
				   if($vo[$resource_field_pid]!=0 && $vo[$resource_field_rootid]!=0 )
				   {
				   ...
				   }elseif($vo[$resource_field_rootid]==0 ) {			   	   
				   	   $apppower[$vo[$resource_field_operate]]=-1;
				   }
			   }
			}
		}
		$_SESSION[self::$config['AUTH_SESSION_PREFIX'].'power']=$power;//设置session
		$_SESSION['yunapppower']=$apppower;//设置app权限
		return $power;//返回权限值
	}
